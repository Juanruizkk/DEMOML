...
async function fetchUserInfo(accessToken) {
  try {
    ...
    if (res.ok) {
      ...
      return { user_id: data.id, nickname: data.nickname };
    }
  } catch (e) {}
  return null;
}
...
async function saveTokens({ access_token, refresh_token, expires_in, user_id }) {
  ...
  let uid = user_id ? String(user_id) : null;
  let nick = null;

  const info = await fetchUserInfo(access_token);
  if (info) {
    if (info.user_id) uid = String(info.user_id);
    if (info.nickname) nick = info.nickname;
  }

  db.prepare(
    `INSERT INTO tokens (id, access_token, refresh_token, expires_at, user_id, nickname, updated_at)
  ...
  ).run(access_token, refresh_token, expiresAt, uid, nick);
  return readTokenRow();
}
...
export function getTokenStatus() {
  ...
  const expectedSellerId = process.env.ML_SELLER_ID;
  const isSeller = !expectedSellerId || String(row.user_id) === String(expectedSellerId);
  return {
    connected: true,
    userId: row.user_id,
